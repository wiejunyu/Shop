...
namespace WL.Home.Manager
{
    public class LoggerManager
    {
        ...
        {
            string sql = "Select * from Logger where Time between @st and @et";
            if(action != "-1")
            {
        ...
        public static LoggerModels GetLoggerModels(string id)
        {
            DynamicParameters param = new DynamicParameters();
            param.Add("@ID", id);
            string sql = "Select * from Logger where ID=@ID";
        ...
        {
            #region sql
            StringBuilder sb = new StringBuilder("Insert into Logger (");
        ...
        {
            string sql = "Select Action from Logger group by Action";
